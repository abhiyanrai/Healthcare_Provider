...
import ConsultationModal from "../../../Pages/Sechedule/ConsultationModal";
import ConsultationForm from "../Consultation/consultaion-form/ConsultationForm";
import { deleteSlotApiDatas, getVisitByPatintIdApi, updateOptionApi } from "../../../Apis";
const Mysechedule = () => {
  ...
  const checkAvailableAppointment = async (date, roomId) => {
    ...
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        let p = res?.data?.appointment?.filter((v) => v.roomId._id === roomId && v.startTime?.split("T")[0] === date);
        p?.filter((a) => {
    ...
    return eventInfo.view.type === "dayGridDay" ? (
      ...
      <div className="table-responsive-md p-2">
        <div className="d-flex flex-row flex-wrap">
          {eventInfo?.event?._def?.extendedProps?.eventData?.map((v) => {
            return (
              // <button
              ...
              >
                <td className="schd-details" onClick={toggleVisibility}>
                  <div className="mysche-room week-bg">
                    <p align="left">
                      <b>
                        {
                          moment(v?.startTime)
        ...
        {isVisible && (
          <div className="right-side-content">
            ...
            <div className="right-side-patient-dts mt-3">
              ...
              <table>
                <thead>
                  ...
                  <tr>
                    <th>D.O.B. :</th>
                    {moment(trackInfo?.patient?.dob).format("DD/MM/YYYY")}
                  </tr>
                  <tr>
                    <th>Email :</th>
            ...
            <div className="right-side-patient-dts mt-5 position-relative">
              ...
              {Boolean(trackInfo?.todaysAppointment?.length) ? (
                trackInfo?.todaysAppointment?.map((v) => {
                  return (
                    <>
                      <hr className="mt-1 mb-2" />
                      <div className="">
                        ...
                        <div className="d-flex">
                          ...
                          {/* <div> 20 min</div> */}
                          <div> {getDurations(v?.startTime, v?.endTime)}</div>
                        </div>
                      </div>
                      <table>
                        <tbody>
            ...
            <div className="right-side-patient-dts mt-5">
              ...
              <div className="table-responsive">
                <table className="next-app-table pb-2">
                  {Boolean(trackInfo?.nextAppointment?.length) ? (
                    trackInfo?.nextAppointment?.map((v) => {
                      return (
                        <tbody
            ...
            <div className="right-side-patient-dts mt-5">
              ...
              {Boolean(trackInfo?.reports?.allConsultation?.length)
                ? trackInfo?.reports?.allConsultation?.map((v) => {
                    return (
              ...
              {Boolean(trackInfo?.reports?.allExamination?.length)
                ? trackInfo?.reports?.allExamination?.map((v) => {
                    return (
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-footer text-end">
              <button
                type="button"
                class="btn btn-sm  btn-light border-base"
                data-bs-dismiss="modal"
              >
                Cancel
              </button>
              <button
                type="button"
                class="btn btn-sm  btn-primary border-base"
