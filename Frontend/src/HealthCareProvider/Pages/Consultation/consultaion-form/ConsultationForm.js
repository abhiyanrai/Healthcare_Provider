...
import { createConsultationApi , getOptionListApiById, imageProfileApi } from "../../../../Apis/healthcareProvider";
    ...
    onSubmit: async (val) => {
      ...
      try {
        ...
        if (res.status === 200 || res.status === 201) {
          ...
          document.getElementsByClassName("btn-close btn-close-con-form")[0].click()
          navigate(`/Consultation/ConsultationView/${res.data.consultation._id}`);
          props.value();
          setclickDis(false)
          resetForm();
        } else {
...
console.log(values.position,"valis[iamdsfojisd")
  ...
  const handleBLtChange = (e) => {
    console.log(values.position,e,"positinino")
    if (check) {
      setCheck(false);
      ...
      <div className="modal-content">
        ...
        <div className="modal-body pt-0">
          <form onSubmit={handleSubmit}>
            <div className="row">
              <div className="col-lg-10 mt-2" onClick={() => getSymptomData(dropdownIds.symptoms)}>
              ...
              <div className="col-lg-4 mt-2" onClick={() => getSymptomData(dropdownIds.radiatesTo)}>
                <span className="font-w">Radiate To: </span>
                <Select
                  name="radiates"
                  options={options1}
                  onChange={(e) => handleInputChange(e, "radiates")}
                  value={Boolean(values?.radiates) ? { label: values?.radiates, value: values?.radiates } : null}
                />
              </div>
              <div className="col-lg-4 mt-2" onClick={() => getSymptomData(dropdownIds.describeSymptom)}>
              ...
              <div className="col-lg-4 col-md-6 mt-2" onClick={() => getSymptomData(dropdownIds.onSet)}>
              ...
              <div className="col-md-6 mt-2" onClick={() => getSymptomData(dropdownIds.palliative)}>
                <span className="font-w">Palliative: </span>
                <CreatableSelect
                  name="palliative"
                  value={{label:values?.palliative , value:values?.palliative}}
                  options={options1}
                  onChange={(e) => handleInputChange(e, "palliative")}
                  noOptionsMessage={() => 'No Data Found'}
                />
              </div>
              <div className="col-md-6 mt-2" onClick={() => getSymptomData(dropdownIds.provocative)}>
                <span className="font-w">Provocative: </span>
                <CreatableSelect
                  name="provocative"
                  value={{label:values?.provocative,value:values?.provocative}}
                  options={options1}
              ...
              <div className="col-lg-6 col-md-12 mt-2" onClick={() => getSymptomData(dropdownIds.therapist)}>
              ...
              <div className="col-lg-6 col-md-12 mt-2" onClick={() => getSymptomData(dropdownIds.help)}>
                ...
                {values?.linkReports && (
                  <span className="d-flex flex-wrap items-center">
                    {values?.linkReports?.map((val, i) => {
                      return <> <a style={{ marginLeft: "10px" }} href={val?.split("")[0] == "h" ? val : `https://${val}`} target="_blank" rel="noreferrer noopener">{val}</a> <span style={{ cursor: "pointer" }} className="ms-2" ><i onClick={() => handlerr(i)} className="bi bi-x-circle"></i></span></>;
                    })}
                  </span>
                )}
              </div>
              <div className="col-lg-6 mt-2">
                ...
                {
                  Boolean(imageData?.length) && imageData?.map((v,i) => {
                    return <p>{v?.split("/")[2]} <span style={{ cursor: "pointer" }} className="ms-2" ><i onClick={() => handleImgDelete(i)} className="bi bi-x-circle"></i></span></p> 
