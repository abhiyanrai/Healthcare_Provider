...
// import { getAllPatientsDetails, getRegularPatientApi } from "../../Apis";
import { getRegularPatientApi } from "../../../Apis/healthcareProvider";
...
const RegularPatient = ({
  ...
  console.log(regularCount , "resgularcounrt")
  ...
  const onChange = async (search) => {
    ...
    try {
      const response = await getRegularPatientApi(
        search,
        currentPage,
        limit
      );
    ...
    <>
      <div className="card rounded-top-0">
        <div className="d-flex card-header border-bottom">
          <h5 className="mb-0 font-bolder h3">Regular Patients<span className="ms-1" style={{fontSize:".9rem",color:"grey"}}>({regularCount})</span></h5>
