...
const digitsWithComa = (value)=>/^\d+(,\d+)*$/.test(value);
export const signUpSchema = Yup.object({
    ...
    password:Yup.string()
    .required("Password can't be empty").min(8,"Password must 8 character long")
...
export const healthcareEditSchema = Yup.object({
  firstName: Yup.string()
  . max(50, "Name should not greater than 50 Characters")
  .matches(/^[aA-zZ\s]*$/, "Only alphabets are allowed for this field ")
  .required("First Name is required").trim('This field cannot include leading and trailing spaces'),
...
export const patientFormSchema = Yup.object().shape({
    ...
    dob: Yup.string().required("Please select the date"),
  ...
  export const consultationFormSchema = Yup.object().shape({
    symptom: Yup.string().required("Please Select Symptom"),
...
export const serviceSchema = Yup.object().shape({
  ...
  // amount:  Yup.string()
  // .test("Digits only", "The field should have digits only", digitsWithComa),
})
...
export const appointmentValidation = Yup.object().shape({
  patientName:Yup.string().required("Please select the pateint name"),
  ...
  appointmentType:Yup.string().required("Please select the appointment type"),
