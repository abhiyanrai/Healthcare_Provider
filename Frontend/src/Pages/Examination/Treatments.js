...
import { getAllAppontmentsApi, getOptionListApiById, getProvidersDetails, getServiceByIdApi, postSlotApiData, updateSlotApiData } from "../../Apis";
...
const Treatments = (props) => {
  ...
  const handleAddShoulderSubtract = () => {
    ...
    if (formik4?.values?.shoulderOption?.length > 0) {
      // formik4?.values?.shoulderOption.splice(formik4?.values?.shoulderOption - 1, 1)
      formik4?.values?.shoulderOption.pop()
    }
    if(formik4?.values?.shoulderOption?.length == 0){
      formik4.setFieldValue("shoulderOption",[{ left: "", leftValue: "", right: "", rightValue: "" }])
    }
    if (render) {
      setRender(false);
    } else {
  ...
  return (
    <>
    <div class="form-ttle py-3"><span></span></div>
      <form onSubmit={formik4.handlSubmit}>

        <div className="row card-header pb-4 pe-5 ps-5">

          {<div className="col-lg-6">
            <h4 className="mt-2 mb-2 col-md-12 d-flex">
              <div className="d-flex align-items-center col-md-10" style={{fontSize: "20px"}}>BMC:
                <span className="text-danger mx-2">{imageName}</span>
              </div>
              <a href="#img" className="d-flex justify-content-end col-md-2">
          ...
          <div className="col-lg-6">
            <h4 className="mt-2 mb-2">
              <div className="d-flex align-items-center col-md-12 " style={{fontSize: "20px"}}>
                Cervical:
                <div className="">
                    <span className="text-danger px-2">{CromNameBooleanFlx(props.formik1)}</span>
                 
               
                    <span className="text-danger  px-2">{CromNameBooleanLlb(props.formik1)}</span>
                  
                
                    <span className="text-danger px-2">{CromNameBooleanProt(props.formik1)}</span>
