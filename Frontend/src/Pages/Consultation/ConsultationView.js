...
const ConsultationView = () => {
  ...
  const [hourView,setHourView]=useState();
  ...
  console.log(imageData, "daiatdajfdskl;jfdsf", values?.attachment);
  ...
  const getCheckExam = async (id) => {
    ...
    console.log(res, "responseifsadlfji");
    if (res.status === 200 || res.status === 201) {
  ...
  const handleAttachChanged = async (e) => {
    ...
    if (!disableds) {
      ...
      try {
        ...
        if (res.status === 200 || res.status === 201) {
          console.log(res.data, "dataattasdfkj");
  ...
  const handleView = async (id) => {
    ...
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        setValues({
          date: res.data.symptom?.date,
          symptom: res.data.symptom?.symptom,
          position: res.data.symptom?.position,
          radiates: res.data.symptom?.radiates,
          describeSymptoms: res.data.symptom?.describeSymptoms,
          begin: res.data.symptom?.begin,
          ago: res.data.symptom?.ago,
          frequency: res.data.symptom?.frequency,
          duration: res.data.symptom?.duration,
          onSet: res.data.symptom?.onSet,
          describeInjury: res.data.symptom?.describeInjury,
          palliative: res.data.symptom?.palliative,
          provocative: res.data.symptom?.provocative,
          warnings: res.data.symptom?.warnings?.length
            ? res.data.symptom?.warnings
            : "",
          additionalNote: res.data.symptom?.additionalNote,
          therapist: res.data.symptom?.therapist,
          help: res.data.symptom?.help,
          linkReports: res.data.symptom?.linkReports,
          attachment: res.data.symptom?.attachment,
          goal: res.data.symptom?.goal,
          id: id,
          blt: res.data.symptom?.blt ? res.data.symptom?.blt : false,
        });
        setHourView(res.data.createdAt);
        console.log(res.data.symptom?.blt, "DASFDSAFDSAFDSAS");
        setImageData(res.data.symptom?.attachment);
      } else {
  ...
  const handleWarningChange = (e) => {
    ...
    // console.log(e,"SADFDASFDSAFSDAF",values?.warnings)
  };
  const getWarnings = () => {
    let arr = [];
  ...
  console.log(warn, "SEDVLSDFISDFSDFSDO");
  useEffect(() => {
                    ...
                    consultaionData?.map((val, index) => {
                      return (
                        <tr>
                          ...
                          <td></td>
                          <td>
                            {console.log(val, "cateatasdfjlakdjfodisf")}
          ...
          <div className="modal-dialog modal-xl">
            <div className="modal-content">
              ...
              <div className="modal-body pt-0">
                {
                  values?.symptom ?  <form onSubmit={handleSubmit}>
                  <div className="row">
                    <div
                      className="col-md-10"
                    ...
                    >
                        ...
                        <label className="font-w">Symptoms:</label>
                    ...
                    <div className="col-lg-2">
                      <label className="font-w"> Date:</label>
                      <input
                    ...
                    <div className="col-md-4 mt-5">
                      <div className="row mt-4">
                        ...
                        {
                          console.log(values?.blt  , "blttvalueeebtl")
                        }
                        <div className="col">
                          <input
                            id="sym-lt"
                            ...
                            name="blt"
                            value={values?.blt == true ? true :false}
                            onChange={()=>setFieldValue("blt",true)}
                            disabled={disableds}
                          />
                          <label for="sym-lt" className="sy-lf-rt-bt">
                            BLT
              ...
              <div className="col-lg-2 col-md-6 mt-2">
                      ...
                      {disableds ? (
                        <span className="d-flex flex-wrap">
                          {values?.linkReports?.map((val) => {
                            return <p style={{ marginLeft: "10px" }}>{val}</p>;
                          })}
                        </span>
                      ) : (
                        values?.linkReports && (
                          <span className="d-flex flex-wrap">
                            {values?.linkReports?.map((val,i) => {
                              return (
                                ...
                                <span style={{ cursor: "pointer" }} className="ms-2" ><i onClick={() => handlerr(i)} className="bi bi-x-circle"></i></span></>
                              );
                            })}
                          </span>
                        )
                ...
                </form> : <div className="row d-flex" style={{justifyContent:"center"}}>
                ...
                <table className="table table-borderless tb_td">
                  <tbody className="d-block">
                    ...
                    <tr>
                      ...
                      {values.begin > 1 ? (
                        <td>{values?.ago ? values.ago + "s" : "-"}</td>
                      ) : (
                        <td>{values?.ago ? values.ago : "-"}</td>
                      )}
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      {values.frequency > 1 ? (
                        <td>
                          {values?.duration ? values.duration + "s" : "-"}
                        </td>
                      ) : (
                        <td>{values?.duration ? values.duration : "-"}</td>
                      )}
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      {console.log(values?.warnings, "weafdnidsaf")}
                      <td>
                        {values.warnings?.length &&
                          values?.warnings?.map((v) => {
                            if (!v?.value) return;
                            return v.value + " , ";
                          })}
                      </td>
